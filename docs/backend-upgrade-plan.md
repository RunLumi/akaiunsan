...
| 2026-09-08 (final) | **Phase 7 PG migration verified**: test config switched to Postgres 16, dialect-aware models/migrator/truncate, PG-compatible keyword search, seed account suite. 442 backend tests + 148 mobile tests green. Coverage 88.5% lines / 63.4% branches / 86.3% functions. All Phases 0–6 complete + Phase 7 infrastructure in place. Remaining: prod data cutover (pgloader), PostGIS/JSONB promotion, TS strictness ramp (~125 typecheck errors), prod deploy secrets (`VPS_HOST` etc.) | — |
| 2026-09-08 (Phase 7 complete) | **Production cutover to PostgreSQL done**: PG 16 + PostGIS (`postgis/postgis:16-3.5-alpine`) live in prod behind `DB_DIALECT=postgres`; migration 003 promotes `address_meta`→jsonb and adds `address.geog geography(Point,4326)` (guarded, PG-only); production rows (role/admin/customer) migrated with verified parity, MariaDB kept as rollback. GitHub Actions secrets `VPS_HOST/VPS_USERNAME/VPS_SSH_KEY/VPS_SSH_PORT` set in repo Settings. NOTE: CI runs are currently blocked by the repo's Actions spending budget (user-side billing), so deploys were executed over SSH in the interim | `5142de5` |
| 2026-09-08 (pins sweep) | **Pinned-bug sweep: 46 of 53 `pins current behavior` markers fixed TDD** (subscription cross-customer leak + cancel-by-id/first-row bug + req.user crash, supporter public-list SELECT destructure, job-remove detail leak via ON DELETE SET NULL order, job-update detail duplication, banner bulk-update crash + transactionality, install first-admin active/role_id, forget-password token payload (password hash in JWT), creditcard TDZ + first-card path, birthday import shadowing ×3, import insert races, permission gate covering collection routes, business errors now carry explicit 400/401/404 instead of blanket 500). Remaining 7 pins are documented legacy shapes (requesthelper data shape, util-csv mapper, agency match endpoints, null-envelope details). 456 tests green ×2, typecheck 0. Branches prod + feat/mobile-app-expo-sdk-57-replatform synced to main; prod redeployed and accepted via /health gate | `78dfe43` |
| 2026-09-08 | **Pinned bug fixed — job create `omise_card_id` ReferenceError** (production `POST /client/jobs` 500 on every create; legacy JS had the same latent bug). `omise_card_id` now destructured from the request body; subscription-consume branch verified end-to-end (used_hour + SubscriptionTransaction). Legacy response contract restored (create returns the job object; updateStatus/matchSupporter keep their deliberate `true`). 455 tests green | `8823753` |
