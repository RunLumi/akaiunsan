...
| 2026-09-08 (Phase 6 complete) | **Phase 6 landed & merged (PR #17, `31dac7a`)**: backend multi-stage Dockerfile (Node 22 builder → slim non-root `node` runner, dist/ + pruned deps + migrations + config, HEALTHCHECK /health); `migrations/` shipped in the image so boot-time migrate-up works on fresh databases; backend CI wired on PRs — MariaDB 12 service runs the full vitest suite, plus a Docker build-validation job; **mobile CI green** (global react-native-swiper stand-in killed the autoplay timers that hung CI workers 10+ minutes; jest per-test timeout raised to 30s). All checks green on PR #17: Test & Validate ✅ · Backend Docker Build ✅ · typecheck·lint·jest ✅. Deploy to VPS remains gated on repo secrets (`VPS_HOST`/`VPS_USERNAME`/`VPS_SSH_KEY`/`VPS_SSH_PORT`) — user-side configuration | `e878793`+ |
