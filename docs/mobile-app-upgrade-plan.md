...
| 2026-09-09 | Phase 3 — any-ratchet + first reduction | New `scripts/count-explicit-any.mjs` (counts `: any`/`as any`/`any[` occurrences in production src — `__tests__`/`test-utils` excluded) with `--check` mode; cap stored in `ci/any-budget.json` and enforced from `ci/check-floors.js` (same gate as coverage; `yarn any:check` standalone). Measured pre-ratchet baseline: **580 occurrences** (575 matching lines incl. tests-era method). First reduction: **typed `useAppSelector` sweep — 26 files, 37 `(state: any)` selectors removed** (including the four ported screens, useApi, BottomTab, navigation root), leaving the cap at 580 with headroom recorded for the next lowering commit. Remaining delta documented in `ci/any-budget.json` (580 to go toward <50; biggest files: Inbox 36, EditAndReOrder 30). Gates: 43/300 green; typecheck 0; lint 0 errors; floors+any OK. |
