...
| 2026-09-08 | Phase 4 — fast-image → expo-image + snap-carousel/swiper → AutoPager/pager-view (PRs #36, #37) | `react-native-fast-image` → `expo-image` 57.0.4 (Home banners/services, PromotionDetail gallery — `contentFit`/string priority); dead deps removed (`react-messenger-customer-chat` web-only, `react-native-restart` — both 0 imports; fast-image itself; doctor exclusion dropped). New shared `AutoPager`: autoplay+loop over `react-native-pager-view` (New-Architecture supported, SDK-pinned 8.0.2 per expo-doctor); all three banner carousels migrated (Home ×2 swiper, PromotionDetail + AllService snap-carousel); **`react-native-snap-carousel` + `react-native-swiper` removed** (+ carousel @types, jest stand-ins). CI-stability: AutoPager autoplay paused under jest (stray intervals kept workers alive past the coverage step). Gates: 36/267 green, typecheck 0, lint 0 errors, **expo-doctor 21/21**. NOTE: pager-view is an autolinked native module — the next CNG prebuild picks it up (native-build validation at that point, per the documented ladder). |
